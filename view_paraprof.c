...
dv_histogram_insert_entry(dv_histogram_t * H, double t, dv_histogram_entry_t * e_hint) {
  ...
  if (e && ee && (ee->t - e->t) < H->min_entry_interval)
    return e;
  dv_histogram_entry_t * new_e = dv_histogram_entry_pool_pop(DMG->epool);
  if (!new_e) {
...
/*
static void
dv_histogram_pile_entry(dv_histogram_t * H, dv_histogram_entry_t * e, dv_histogram_layer_t layer, double thick, dm_dag_node_t * node) {
  (void)H;
  (void)node;
  e->h[layer] += thick;
}
...
static void
dv_histogram_pile(dv_histogram_t * H, dm_dag_node_t * node, dv_histogram_layer_t layer, double from_t, double to_t, double parallelism) {
...
void
dv_histogram_add_node(dv_histogram_t * H, dm_dag_node_t * node, dv_histogram_entry_t ** hint_entry) {
  if (!H)
    return;
...
/*
static void
dv_histogram_unpile_entry(dv_histogram_t * H, dv_histogram_entry_t * e, dv_histogram_layer_t layer, double thick, dm_dag_node_t * node) {
  (void)H;
  (void)node;
  e->h[layer] -= thick;
}
...
static void
dv_histogram_unpile(dv_histogram_t * H, dm_dag_node_t * node, dv_histogram_layer_t layer, double from_t, double to_t, double parallelism) {
  dv_histogram_entry_t * e_from = dv_histogram_insert_entry(H, from_t, NULL);
...
void
dv_histogram_remove_node(dv_histogram_t * H, dm_dag_node_t * node, dv_histogram_entry_t ** hint_entry) {
  if (!H)
    return;
  dr_pi_dag_node * pi = dm_pidag_get_node_by_dag_node(H->D->P, node);
  double from_t, to_t, dt;
...
dv_histogram_clean(dv_histogram_t * H) {
  ...
  while (e != NULL) {
    ee = e->next;
    dv_histogram_entry_pool_push(DMG->epool, e);
    n++;
    e = ee;
  }
  dv_check(n == H->n_e);
...
static void
dv_histogram_reset_node(dv_histogram_t * H, dm_dag_node_t * node, dv_histogram_entry_t * e_hint_) {
  /* Calculate inward */
...
static void
dv_histogram_build_all_r(dv_histogram_t * H, dm_dag_node_t * node, dv_histogram_entry_t * e_hint_) {
  /* Calculate inward */
  dv_histogram_entry_t * e_hint = e_hint_;
  if (dm_is_union(node) && dm_is_inner_loaded(node)) {
...
_unused_ static int
dv_histogram_entry_is_invisible(dm_histogram_t * H, dv_view_t * V, dm_histogram_entry_t * e) {
  int ret = -1;
  if (!H || !V || !e) return ret;
...
_unused_ static int
dv_histogram_entry_is_invisible_fast(dm_histogram_t * H, dv_view_t * V, dm_histogram_entry_t * e) {
  double x, w;
  x = dv_dag_scale_down_linear(H->D, e->t - H->D->bt);
  w = 0.0;
  if (e->next)
...
static double
dv_histogram_draw_piece(_unused_ dm_histogram_t * H, dv_view_t * V, cairo_t * cr, double x, double width, double y, double height, int layer) {
  cairo_save(cr);
...
static void
dv_histogram_draw_entry(dm_histogram_t * H, dm_histogram_entry_t * e, cairo_t * cr, dv_view_t * V) {
  if (!e->next) {
...
dv_histogram_cal_work_delay_nowork(dm_histogram_t * H) {
  H->work = H->delay = H->nowork = 0.0;
...
void
dv_histogram_draw(dm_histogram_t * H, cairo_t * cr, dv_view_t * V) {
  double time = dm_get_time();
  ...
  cairo_set_line_width(cr, DVG->opts.nlw / V->S->zoom_ratio_x);
  cairo_stroke(cr);
  cairo_restore(cr);
...
static void
dv_paraprof_draw_time_bar(dv_view_t * V, dm_histogram_t * H, cairo_t * cr) {
  double x = dv_dag_scale_down_linear(H->D, H->D->current_time);
  double y1 = - dm_histogram_get_max_height(H) - H->D->radius;
  double y2 = H->D->P->num_workers * (H->unit_thick * H->D->radius) + H->D->radius;
  cairo_save(cr);
  cairo_move_to(cr, x, y1);
  cairo_line_to(cr, x, y2);
  cairo_set_source_rgba(cr, 0.0, 0.0, 0.0, 0.8);
  cairo_set_line_width(cr, (DVG->opts.nlw * 2) / V->S->zoom_ratio_x);
  cairo_stroke(cr);
  cairo_restore(cr);
}
...
dv_paraprof_convert_graph_x_to_paraprof_x(dm_histogram_t * H, double x) {
  return x * H->D->linear_radix;
}

/* (normalized) time point -> graph x */
_static_unused_ double
dv_paraprof_convert_paraprof_x_to_graph_x(dm_histogram_t * H, double x) {
  return dv_dag_scale_down_linear(H->D, x);
}

/* graph y -> parallelism/worker */
_static_unused_ double
dv_paraprof_convert_graph_y_to_paraprof_y(dm_histogram_t * H, double y) {
  if (y < 0) {
    /* to parallelism */
...
/* parallelism -> graph y */
_static_unused_ double
dv_paraprof_convert_parallelism_to_graph_y(dm_histogram_t * H, double p) {
...
_static_unused_ double
dv_paraprof_convert_worker_number_to_graph_y(dm_histogram_t * H, double w) {
  return w * (H->D->radius * 2);
}

_static_unused_ double
dv_paraprof_convert_viewport_x_to_paraprof_x(dm_histogram_t * H, dv_view_t * V, double x) {
  return dv_paraprof_convert_graph_x_to_paraprof_x(H, dv_view_convert_viewport_x_to_graph_x(V, x));
}
_static_unused_ double
dv_paraprof_convert_paraprof_x_to_viewport_x(dm_histogram_t * H, dv_view_t * V, double x) {
  return dv_view_convert_graph_x_to_viewport_x(V, dv_paraprof_convert_paraprof_x_to_graph_x(H, x));
}
_static_unused_ double
dv_paraprof_convert_viewport_y_to_paraprof_y(dm_histogram_t * H, dv_view_t * V, double y) {
  return dv_paraprof_convert_graph_y_to_paraprof_y(H, dv_view_convert_viewport_y_to_graph_y(V, y));
}
_static_unused_ double
dv_paraprof_convert_parallelism_to_viewport_y(dm_histogram_t * H, dv_view_t * V, double p) {
  return dv_view_convert_graph_y_to_viewport_y(V, dv_paraprof_convert_parallelism_to_graph_y(H, p));
}
_static_unused_ double
dv_paraprof_convert_worker_number_to_viewport_y(dm_histogram_t * H, dv_view_t * V, double w) {
...
dv_paraprof_draw_rulers(dv_viewport_t * VP, dv_view_t * V, cairo_t * cr) {
  ...
  /* vertical ruler: upper part */
  {
    ...
    double tick_interval_2 = tick_interval / 5.0;
    double tick_interval_3 = tick_interval / 10.0;
    double y_top = dv_min(dm_histogram_get_max_height(H) / (H->unit_thick * H->D->radius), dv_paraprof_convert_viewport_y_to_paraprof_y(H, V, ruler_width));
    double y_bottom = 0.0;
...
dv_critical_path_find_clicked_node_r(dv_view_t * V, double cx, double cy, dm_dag_node_t * node) {
...
dv_critical_path_find_clicked_node(dv_view_t * V, double x, double y) {
  dm_dag_node_t * ret = dv_critical_path_find_clicked_node_r(V, x, y, V->D->rt);
  return ret;
}

static void
dv_view_layout_critical_path_node(dv_view_t * V, dm_dag_node_t * node, int * parent_on_global_cp) {
  /* Counting statistics */
  V->S->nl++;
  ...
  /* Calculate link-along */
  dm_dag_node_t * next = NULL;
  while ( (next = (dm_dag_node_t *) dm_dag_node_traverse_nexts(node, next)) ) {
...
static void
dv_view_draw_critical_path_node_1(dv_view_t * V, cairo_t * cr, dm_dag_node_t * node, int * on_global_cp) {
  cairo_save(cr);
  /* Get inputs */
  dm_dag_t * D = V->D;
  dv_view_status_t * S = V->S;
  dm_node_coordinate_t * nodeco = &node->c[S->coord];
  ...
  for (cp = 0; cp < DV_NUM_CRITICAL_PATHS; cp++) {
    ...
    //if ( (D->show_critical_paths[cp] && dm_node_flag_check(node->f, DMG->oncp_flags[cp])) ) {
    if (on_global_cp[cp]) {
      ...
      if (!dv_rectangle_is_invisible(V, xx, yy, w, h)) {
        ...
        if (dm_is_union(node)) {
          ...
          if (1) {
            ...
            if (line_width > 10 * DVG->opts.nlw)
              line_width = 10 * DVG->opts.nlw;
...
static void
dv_view_draw_critical_path_node_r(dv_view_t * V, cairo_t * cr, dm_dag_node_t * node, int * parent_on_global_cp) {
  /* Counting statistics */
  V->S->ndh++;
