...
dv_timeline_find_clicked_node_r(dv_view_t * V, double cx, double cy, dm_dag_node_t * node) {
  ...
  dm_node_coordinate_t * co = &node->c[coord];
  /* Call inward */
  double x, y, w, h;
  x = co->x;
  y = co->y;
...
dv_timeline_find_clicked_node(dv_view_t * V, double x, double y) {
  dm_dag_node_t * ret = dv_timeline_find_clicked_node_r(V, x, y, V->D->rt);
  return ret;
}
...
int
dv_timeline_node_is_invisible(dv_view_t * V, dm_dag_node_t * node) {
  int ret = -1;
  if (!node) return ret;
...
static void
dv_view_layout_timeline_node(dv_view_t * V, dm_dag_node_t * node) {
  V->S->nl++;
...
static void
dv_view_draw_timeline_node_1(dv_view_t * V, cairo_t * cr, dm_dag_node_t * node, int * on_global_cp) {
  cairo_save(cr);
  /* Get inputs */
  dm_dag_t * D = V->D;
  ...
  if (!dv_timeline_node_is_invisible(V, node)) {
    ...
    if (dm_is_union(node)) {
      ...
      /* Highlight critical paths */
      {
        ...
        if (line_width > 40 * DVG->opts.nlw)
          line_width = 40 * DVG->opts.nlw;
...
static void
dv_view_draw_timeline_node_r(dv_view_t * V, cairo_t * cr, dm_dag_node_t * node, int * parent_on_global_cp) {
  /* Counting statistics */
  V->S->ndh++;
